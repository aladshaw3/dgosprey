...
void GasFlowProperties::computeQpProperties()
{
	...
	double ReNum = _velocity[_qp] * _inner_dia[_qp] / kin_vis;
	double pre_coef = _velocity[_qp] * _pellet_diameter[_qp] / _porosity[_qp];
	for (unsigned int i = 0; i<_gas_conc.size(); ++i)
	{
		...
		//_dispersion[_qp][i] = (_porosity[_qp] * _molecular_diffusion[_qp][i] + (10.0*_pellet_diameter[_qp]*_velocity[_qp]));
	...
	_conductivity[_qp] = PeNum * _gas_heat_capacity[_qp] * _gas_density[_qp] * _velocity[_qp] * _pellet_diameter[_qp];
